...
void RenderSystem::queryHardwareCaps() {
	...
	glGetFloatv(GL_MAX_TEXTURE_MAX_ANISOTROPY, &m_caps.MaxAnisotropy);
...
void RenderSystem::setupTextureSamplers() {
	...
	glSamplerParameterf(m_samplerPBRTextures, GL_MAX_TEXTURE_MAX_ANISOTROPY, m_caps.MaxAnisotropy);
...
void RenderSystem::setupShadowMap() {
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_BORDER); // Clamp to border to fix over-sampling
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_BORDER);
	glTexParameterf(GL_TEXTURE_2D, GL_MAX_TEXTURE_MAX_ANISOTROPY, m_caps.MaxAnisotropy); // Anisotropic filtering for sharper angles
	glTexParameterfv(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR, borderColor);
...
void GLDebugMessageCallback(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar * msg, const void * data)
{
    ...
    printf("|||OPENGL ISSUE||| ID: (%d), Type: (%s), Severity : (%s), raised from %s --> %s\n",
